...
const generateTeam = (): ITeam => {
  const players = Array.from({ length: faker.number.int({ min: 1, max: 5 }) }, () => generatePlayer())
...
describe('SrConverterService', () => {
  it('should convert data to the desired format', () => {
    ...
    fs.writeFileSync('result.json', JSON.stringify(convertedData, null, 4));

    expect(convertedData).toBeTruthy();
    expect(convertedData.game.id).toEqual(correctData.game.id);
    expect(convertedData.game.attendance).toEqual(correctData.game.attendance.toString());
    // expect(Object.keys(convertedData.teams)).toHaveLength(2);
    // expect(Object.keys(convertedData.players)).toHaveLength(10);
  });
  ...
  it('should throw an error if data format does not match', () => {
    ...
    expect(() => new SrConverterService(invalidSourceData as ISourceData)).toThrow(errors.dataFormatDoesNotMatch(SERVICE_NAME));
  });
  ...
  it('should throw an error for duplicate players', () => {
    const sourceData = createFakeSourceData();
    sourceData.statistics.home.rushing.players[0].id = sourceData.statistics.away.rushing.players[0].id;
